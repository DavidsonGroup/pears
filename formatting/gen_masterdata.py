...
def gene_range(shr_output, gene_dict):
    ...
    df.insert(4, "sequence1", None)
    df.insert(7, "gene2", None)
    df.insert(10, "sequence2", None)
...
def get_flexi_sequences(df, len_barcode, fasta):
    ...
    gene2_seq = []
    for index, row in df.iterrows(): 
        gene1_seq.append(get_sequence([row['chrom1'], row['base1'] - len_barcode, row['base1']], fasta))
        gene2_seq.append(get_sequence([row['chrom2'], row['base2'], row['base2'] + len_barcode], fasta))
...
os.system('echo gene done')
fasta = reference + '/fasta/genome.fa'
os.system('echo fasta done')
gene_dict = get_gene_dict(shr_output)
os.system('echo gene_range done')
...
flexi_gene_range = get_flexi_sequences(fuscia_gene_range, 20, fasta)
