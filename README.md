...
| `--demultiplexer` | `direct` | How barcodes are assigned. `direct` reads the barcode and UMI at their fixed offset and corrects against candidates taken from the reads; `flexiplex` hands the barcode list to Flexiplex instead. Visium HD and `--demultiplex_all_reads` always use Flexiplex. |
...
| `--barcode_discovery_top_n` | `50000` | Only with `--demultiplex_all_reads`: the most abundant observed barcodes kept from the Flexiplex discovery pass before intersecting with the whitelist. |
| `--demultiplex_all_reads` | `false` | Demultiplex every read in the library rather than only the reads fusion calling needs. Much slower — Flexiplex compares every read that is not an exact match against the entire barcode list. Required by `--tag_full_bam`. |
| `--tag_full_bam` | `false` | Write `CB`/`UB` tags onto every read in the BAM rather than only reads over the fusion targets. Needs `--demultiplex_all_reads`, and enough memory to hold the whole read ID to barcode table. |
...
| `demultiplex/` | Flexiplex barcode table (`reads_barcodes.txt`), the discovered barcode counts and the barcode list used, and the read IDs and R1 reads that were demultiplexed (`target_read_ids.txt`, `target_R1.fastq`). |
